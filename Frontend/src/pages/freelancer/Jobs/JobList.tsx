...
const FreelancerJobListing = () => {
    ...
    // Helper functions for mock data display
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            case 'Ongoing': return <Clock className="w-3 h-3 mr-1" />;
            case 'Completed': return <CheckCircle className="w-3 h-3 mr-1" />;
            case 'Bids': return <FileText className="w-3 h-3 mr-1" />;
            default: return null;
        }
    };
                            ...
                            >
                                {activeTab === 'All' ? (
                                    ...
                                    <div className="flex flex-col lg:flex-row gap-8">
                                        ...
                                        {/* Right Sidebar - Filters (Only for All tab) */}
                                                ...
                                                >
                                                    <div className="flex justify-between items-start">
                                                        <div className="flex gap-4">
                                                            <div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-gray-500 group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors">
                                                                <Briefcase className="w-6 h-6" />
                                                            </div>
                                                            <div>
                                                                <h3 className="font-semibold text-gray-900 group-hover:text-emerald-600 transition-colors">{project.title}</h3>
                                                                <p className="text-sm text-gray-500">{project.client}</p>
                                                            </div>
                                                        </div>
                                                        <div className="flex flex-col items-end gap-2">
                                                            <span className={`px-2.5 py-1 rounded-full text-xs font-medium border flex items-center ${getStatusColor(project.status)}`}>
