...
import { freelancerChangePassword } from '../../../service/freelancer/authService'
...
const ChangePassword = () => {
  ...
  if (userdata?.googleId) {
    return (
      <div className="p-8 max-w-2xl mx-auto text-center mt-20">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Password Managed by Google</h1>
        <p className="text-gray-500 mt-2">
          Your account is authenticated via Google. Password changes should be managed through your Google Account settings.
        </p>
      </div>
    )
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      const response = await freelancerChangePassword({ currentPassword, newPassword })
